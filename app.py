...
st.markdown("<h1 style='text-align: center; color: blue;'>Youtube Data Harvesting</h1>", unsafe_allow_html=True)
...
with colS2:link = st.text_input(" :orange[Enter The Youtube Channel link]")
    
colB1,colB2 = st.columns([10,10])
with colB2:searc = st.button('search')
...
if searc:
    if link:
        ...
        with colc4:st.image(channel_data['Thumbnail'], width=200)
        ...
        with colB3:mang_bt = st.button(' :green[ upload to mangodb ]',on_click = mdb_insert(channel_data['Channel_Name'],channel_data,pd.DataFrame(get_video_info),pd.DataFrame(comment_df)))
        with colB2:sql_bt = st.button(' :green[ upload to sqldb ]',on_click=update_mysql(channel_data,get_video_info,comment_df))
